    ...
    for (const item of queue) {
      try {
        if (item.action === "clockIn") {
          await this.clockIn(item.payload, true);
        }
        ...
        if (item.action === "startBreak") {
          await this.startBreak(true);
        }

        if (item.action === "endBreak") {
